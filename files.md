...
| File | Description |
|------|-------------|
| `index.js` | Main scraper - extracts jobs from EPAM careers page (Romania), validates locations, saves jobs.json |
| `company.js` | Validates company via ANAF + Peviitor APIs, checks if company is active/inactive, imports SOLR functions from solr.js |
| `solr.js` | SOLR operations module - exports querySOLR, deleteJobsByCIF, upsertJobs (used by company.js) |
...
| File | Description |
|------|-------------|
| `jobs.json` | Transformed jobs ready for SOLR upsert (generated by index.js) |
...
- Workflow is: SOLR check → ANAF validation → Peviitor validation → company status check → scrape → transform → save jobs.json